...
import { alterarSenha } from '../api'; // Função de alteração de senha
...
const AlterarSenhaPage = () => {
  ...
  const handleAlterarSenha = async (e: React.FormEvent) => {
    ...
    try {
      const status = await alterarSenha(cpf, senhaAtual, novaSenha);
      if (status === 200) {
  ...
  const handleModalClose = () => {
    ...
    navigate('/parcelas'); // Redireciona para a tela de login após fechar o modal
  };
